...
#else
#define TRACKLE_ASSERT(predicate) assert(predicate)
#endif

#if defined(DEBUG_BUILD) && defined(DEBUG_THREADING) && PLATFORM_THREADING == 1
#if defined(MODULE_INDEX) && MODULE_INDEX == 1
#define ASSERT_ON_SYSTEM_THREAD() TRACKLE_ASSERT(system_thread_current_(nullptr))
#define ASSERT_ON_SYSTEM_OR_MAIN_THREAD() TRACKLE_ASSERT(system_thread_current_(nullptr) || main_thread_current_(nullptr))
#else
#define ASSERT_ON_SYSTEM_THREAD() TRACKLE_ASSERT(system_thread_current(nullptr))
#define ASSERT_ON_SYSTEM_OR_MAIN_THREAD() TRACKLE_ASSERT(system_thread_current(nullptr) || main_thread_current(nullptr))
#endif
#else
