...
namespace trackle
{
	namespace protocol
	{
		...
		class Protocol
		{
			...
			public:
				void init(TrackleCallbacks *callbacks)
				{
			...
			/**
			 * Manages device-hosted properties.
			 */
      ...
      ProtocolError wait_ack(message_id_t id);
			 ...
			 * @retval \p trackle::protocol::NO_ERROR
			 *
			 * @sa trackle::protocol::ProtocolError
			 */
			...
			void init(const TrackleCallbacks &callbacks, const TrackleDescriptor &descriptor);
		...
		public:
			...
			bool time_request_pending() const
			{
				return timesync_.is_request_pending();
			}
			system_tick_t time_last_synced(time_t *tm) const
			{
				return timesync_.last_sync(*tm);
			}
