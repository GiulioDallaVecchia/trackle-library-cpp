...
namespace trackle
{
    namespace protocol
    {
            ...
            token_t token, Message &message, MessageChannel &channel)
        {
            ...
            if (actual_len >= 20 && queue[7] == 0xFF)
            {
                ...
                if (fast_ota_override)
                {
                    ...
                    TRACKLE_DEBUG("Fast OTA: %s", fast_ota_value ? "enabled" : "disabled");
                }
            ...
            if (success)
            {
                ...
                {
                    TRACKLE_DEBUG("starting file length %d chunks %d chunk_size %d",
                    ...
                    if (error)
                        TRACKLE_DEBUG("error sending updateReady");
                }
            }
            return error;
        }
                                                    ...
                                                    MessageChannel &channel)
        {
            ...
            if (!is_updating())
            {
                TRACKLE_WARN("got chunk when not updating");
            ...
            if (0xFF == queue[payload])
            {
                ...
                {
                    TRACKLE_WARN("invalid chunk index %d", chunk_index);
                    return NO_ERROR;
                }
                uint32_t crc = callbacks->calculate_crc(chunk, file.chunk_size);
                uint16_t response_size = 0;
                bool crc_valid = (crc == given_crc);
                TRACKLE_DEBUG("chunk idx=%d crc=%d fast=%d updating=%d", chunk_index,
                ...
                {
                    TRACKLE_WARN("chunk crc bad %d: wanted %x got %x", chunk_index, given_crc, crc);
                    if (!fast_ota)
                    {
        ...
        ProtocolError ChunkedTransfer::handle_update_done(token_t token, Message &message, MessageChannel &channel)
        {
            ...
            TRACKLE_DEBUG("update done received");
            ...
            if (!missing)
            {
                TRACKLE_DEBUG("update done - all done!");
                reset_updating();
            ...
            else
            {
                ...
                TRACKLE_DEBUG("update done - missing chunks starting at %d", index);
                                                           ...
                                                           size_t count)
        {
            ...
            if (sent > 0)
            {
                TRACKLE_DEBUG("Sent %d missing chunks", sent);
        ...
        void ChunkedTransfer::cancel()
        {
            if (is_updating())
            {
                ...
                TRACKLE_WARN("handle received message failed - aborting transfer");
